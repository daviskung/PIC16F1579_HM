...
void Cal_HeartRate(void)
{
	...
	if(AN2_AryAvg_value > AN2_ChkValue){
		...
		if( AN2_HRBufferIndex < AN2HR_BUF_SIZE ){
			AN2_HRBuffer[AN2_HRBufferIndex] = AN2_tmpPulseCnt;
			AN2_HRBufferIndex++;
			
			AN2_READ_PulseIntervalAvg_1msCnt = AN2_tmpPulseCnt;
			
		}
		else{
			AN2_HRBuffer[0] = AN2_HRBuffer[1];
			AN2_HRBuffer[1] = AN2_HRBuffer[2];
			AN2_HRBuffer[2] = AN2_HRBuffer[3];
			AN2_HRBuffer[3] = AN2_HRBuffer[4];	
			AN2_HRBuffer[4] = AN2_tmpPulseCnt;
			
			//DugDataMsg('-','h','r', AN2_tmpPulseCnt );
			
			AN2_PulseIntervalAvg = AN2_HRBuffer[0];
			for (i = 1; i < (AN2HR_BUF_SIZE) ; ++i)
				AN2_PulseIntervalAvg = AN2_PulseIntervalAvg + AN2_HRBuffer[i];
			AN2_PulseIntervalAvg = AN2_PulseIntervalAvg/(AN2HR_BUF_SIZE);
			AN2_READ_PulseIntervalAvg_1msCnt = AN2_PulseIntervalAvg;
			
			//DugDataMsg('2','h','r', AN2_READ_PulseIntervalAvg_1msCnt );
		}
...
void main(void)
{
	...
	DugCmdMsg('2','7','d'); // version number day is // test 0417 for hex file
